...
export function buildFinanceToolSpec() {
    const spec = {
        ...
        function: {
            name: 'finance_calc',
            description: 'Deterministic financial calculations for SaaS/B2B business models. Computes unit economics (ARPU, Gross Margin, CAC, LTV, Payback) and generates 12-24 month revenue/customer projections with churn modeling.',
            parameters: {
                type: 'object',
                properties: {
                    // Pricing & costs
                    ...
                    expansionPctMonthly: {
                        type: 'number',
                        description: 'Monthly expansion/upsell rate on retained customers (0-1 scale, default 0)',
                        minimum: 0,
                        maximum: 1,
                    },
                    // Sales/marketing & CAC
                    ...
                    customersAcquiredPerMonth: {
                        type: 'number',
                        description: 'Customers acquired per month for CAC calc (if different from newCustomersPerMonth)',
                        minimum: 0,
                    },
                    assumedCAC: {
                        type: 'number',
 ...
 */
export function getFinanceToolHandler() {
    const { handler } = buildFinanceToolSpec();
    return handler;
}
//# sourceMappingURL=financeTool.js.map