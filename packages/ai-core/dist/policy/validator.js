 ...
 */
import { validateCitations as validateCitationsInternal, findSection, } from './citations.js';
 ...
 */
function validateRecency(response, webSources, requiresWebSearch, recentMonths) {
    const issues = [];
 ...
 * @returns Validation issues
 */
function validatePII(response, blockPII) {
    const issues = [];
    if (!blockPII) {
        return issues;
    }
 ...
 * @returns Validation issues
 */
function validateRequiredSections(response, requiredSections) {
    const issues = [];
 ...
 */
export function validateOutput(response, context) {
    ...
    // 2. Number validation
    const numberIssues = validateNumbers(context.assistant, response.rawModelOutput, context.financeModel, config.strictNumbers);
    ...
    const recencyIssues = validateRecency(response, webSources, context.requiresWebSearch || false, config.recentMonths);
 ...
 * @returns Validation result
 */
export function validateCitations(response) {
    const config = loadPolicyConfig();
 ...
 * @returns Validation result
 */
export function validateFactualConsistency(response, context) {
    const issues = [];
    // Check for unsupported numbers
    const numberIssues = validateNumbers('streamlined_plan', // Default to general validation
    response.rawModelOutput, context.financeModel);
    issues.push(...numberIssues);
    return {
        valid: !issues.some((i) => i.severity === 'error'),
        issues,
 ...
 * @returns Validation result
 */
export function validateFinancialMetrics(response, context) {
    const config = loadPolicyConfig();
    const issues = validateNumbers('financial_overview', response.rawModelOutput, context.financeModel, config.strictNumbers);
    return {
        valid: !issues.some((i) => i.severity === 'error'),
        issues,
    };
