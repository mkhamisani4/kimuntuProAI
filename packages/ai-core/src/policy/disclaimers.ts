 ...
 */
function buildFinancialDisclaimer(hasFinanceData: boolean): string {
  const disclaimer = `**Financial Disclaimer**

This document contains forward-looking financial projections and estimates${
    hasFinanceData ? ' based on provided assumptions' : ''
  }. Actual results may vary significantly. This analysis is for informational purposes only and does not constitute financial, investment, or legal advice.
 ...
 * @returns Disclaimer text
 */
function buildMarketDisclaimer(webSources: AssistantSource[]): string {
 ...
 */
function buildIssueWarning(issues: ValidationIssue[]): string {
  ...
  const parts: string[] = ['**Quality Notice**'];

  if (errorCount > 0) {
    parts.push(
      `This response has ${errorCount} error${errorCount > 1 ? 's' : ''} that should be addressed:`
    );
  }

  if (warningCount > 0) {
    parts.push(
      `${errorCount > 0 ? 'Additionally, there are' : 'There are'} ${warningCount} warning${
        warningCount > 1 ? 's' : ''
      }:`
    );
  }
 ...
 */
function getIssueSummary(code: string, count: number): string {
 ...
 */
function groupIssuesByCode(issues: ValidationIssue[]): Record<string, ValidationIssue[]> {
 ...
 */
function findLatestSourceDate(webSources: AssistantSource[]): Date | null {
