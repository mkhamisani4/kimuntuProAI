 ...
 */
export function buildOpenAIWebSearchTools(): any[] {
  const config = loadConfig();
 ...
 */
function filterResults(results: WebSearchResult[], config: WebSearchConfig): WebSearchResult[] {
 ...
 */
function extractSearchResults(content: string): WebSearchResult[] {
...
export async function webSearchWithOpenAI(
  ...
  if (!allowed) {
    throw new Error(
      `Rate limit exceeded for tenant ${options.tenantId}. Please try again later.`
    );
  }
