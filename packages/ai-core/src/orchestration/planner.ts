 ...
 */
export function deriveHeuristics(input: PlannerInput): PlanHeuristics {
  const { assistant, input: userInput } = input;

  // Initialize heuristics
  const heuristics: PlanHeuristics = {
 ...
 */
function buildFallbackPlan(input: PlannerInput, heuristics: PlanHeuristics): PlannerOutput {
  return {
...
export async function plan(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const response = await aiClient.chatStructured<PlannerOutput>({
      ...
      if (
        (validated.requires_retrieval || validated.requires_web_search) &&
      ...
      if (attempt === 0 && error.name === 'ZodError') {
        messages.push({
          role: 'user' as const,
          content:
            'Schema violation detected. Please output ONLY valid JSON matching PlannerOutput schema with no extra fields or markdown.',
        });
        continue;
      }
  ...
  console.warn(
    `Planner LLM failed after ${maxAttempts} attempts:`,
 ...
 */
export function validatePlannerInput(input: unknown): {
  success: boolean;
  data?: PlannerInput;
  errors?: string[];
} {
