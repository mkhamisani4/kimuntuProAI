 ...
 */
export async function listWebsites(tenantId, userId, limitCount = 20) {
    try {
        let q;
        if (userId) {
            q = query(collection(db, 'websites'), where('tenantId', '==', tenantId), where('userId', '==', userId), orderBy('createdAt', 'desc'), limit(limitCount));
        }
        else {
            q = query(collection(db, 'websites'), where('tenantId', '==', tenantId), orderBy('createdAt', 'desc'), limit(limitCount));
        }
