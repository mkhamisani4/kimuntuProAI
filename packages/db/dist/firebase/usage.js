 ...
 */
import { db, Timestamp, collection, addDoc, query, where, orderBy, getDocs, } from './client.js';
/**
...
export async function sumTokensByUser(options) {
    ...
    try {
        const q = query(collection(db, 'usage_logs'), where('userId', '==', userId), where('createdAt', '>=', Timestamp.fromDate(since)));
...
export async function sumTokensByTenant(options) {
    ...
    try {
        const q = query(collection(db, 'usage_logs'), where('tenantId', '==', tenantId), where('createdAt', '>=', Timestamp.fromDate(since)));
        const snapshot = await getDocs(q);
        const total = snapshot.docs.reduce((sum, doc) => {
            const data = doc.data();
            return sum + (data.tokensIn || 0) + (data.tokensOut || 0);
