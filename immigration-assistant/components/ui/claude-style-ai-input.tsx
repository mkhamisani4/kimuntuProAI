...
const isTextualFile = (file: File): boolean => {
  ...
  const textualExtensions = [
    ...
    "astro","config","conf","ini","toml","log","gitignore","dockerfile",
    "makefile","readme",
  ];
...
const ClaudeChatInput: React.FC<ChatInputProps> = ({
  ...
  const canSend =
    hasContent && !disabled && !files.some((f) => f.uploadStatus === "uploading");
