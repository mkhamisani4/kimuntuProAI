...
/**
 * Business Law AI Assistant
 * Provides guidance on business formation, corporate compliance, IP, employment law, and commercial regulations
 */
...
export default function BusinessLawAssistant() {
    ...
    const { config: avatarConfig, updateConfig: updateAvatar, resetConfig: resetAvatar } = useAIAvatar('business');
    ...
    const handleChatSend = async (message: string, categoryOverride?: string) => {
        ...
        try {
            ...
            const response = await fetch('/api/legal/business/ask', {
                method: 'POST',
                headers,
                body: JSON.stringify({
            ...
            if (!response.ok) {
                throw new Error(data.error || 'Failed to get business law guidance');
            }
    ...
    const quickQuestions = [
        { question: 'What are the steps to form an LLC?', category: 'Business Formation (LLC, Corp, Partnership)' },
        { question: 'What are my obligations as a corporate director?', category: 'Corporate Compliance & Governance' },
        { question: 'How do I protect my business name with a trademark?', category: 'Intellectual Property (Trademarks, Patents, Copyrights)' },
        { question: 'What employment laws apply to small businesses?', category: 'Employment Law & HR' }
    ];
        ...
        <div className={`min-h-screen p-4 md:p-8 ${isDark ? 'bg-gray-950' : 'bg-gray-50'}`}>
            <div className="max-w-6xl mx-auto">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                    Guidance on business formation, corporate compliance, IP, and commercial law
                                </p>
                            </div>
                        </div>
                        <div className={`px-4 py-2 rounded-lg ${isDark ? 'bg-amber-500/20 border border-amber-500/40' : 'bg-amber-50 border border-amber-200'}`}>
                            <span className={`font-medium ${isDark ? 'text-amber-300' : 'text-amber-700'}`}>Business & Corporate</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className={`rounded-2xl p-6 ${isDark ? 'bg-gray-900/80 border border-gray-800' : 'bg-white border border-gray-200'} shadow-lg`}>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className={`w-full px-4 py-3 rounded-lg border ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-300 text-gray-900'} focus:ring-2 focus:ring-amber-500 focus:border-transparent`}
                                    >
                            ...
                            <div className="mt-4">
                                <ClaudeChatInput
                                    ...
                                    disabled={loading}
                                    placeholder="e.g., What are the requirements to form an LLC in my state?"
                                    models={[]}
                                />
                            </div>
                        </div>
                            ...
                            <div className={`mt-6 rounded-2xl p-6 ${isDark ? 'bg-gray-900/80 border border-gray-800' : 'bg-white border border-gray-200'} shadow-lg`}>
                                <div className="flex items-start gap-4 mb-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${isDark ? 'bg-amber-500/20 text-amber-300' : 'bg-amber-50 text-amber-700'}`}>
                                            {result.category}
                                        </span>
                                    </div>
                                </div>
                                            ...
                                            {result.sources.map((source, idx) => (
                                                <div key={idx} className={`p-3 rounded-lg ${isDark ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
                                                    <div className="flex items-start gap-2">
                                                        <CheckCircle className={`w-4 h-4 mt-0.5 flex-shrink-0 ${isDark ? 'text-amber-400' : 'text-amber-600'}`} />
                                                        <div className="flex-1">
                                                            ...
                                                            {source.url && (
                                                                <a href={source.url} target="_blank" rel="noopener noreferrer" className={`text-xs mt-1 inline-block ${isDark ? 'text-amber-400 hover:text-amber-300' : 'text-amber-600 hover:text-amber-700'}`}>
                                                                    View source →
                                                                </a>
                                                            )}
                                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {result.relatedTopics.map((topic, idx) => (
                                                <button
                                                    ...
                                                    className={`px-3 py-1 rounded-full text-xs border transition-colors ${isDark ? 'border-amber-500/30 text-amber-300 hover:bg-amber-500/10 hover:text-amber-200' : 'border-amber-200 text-amber-700 hover:bg-amber-50'}`}
                                                >
                                                    {topic}
                                                </button>
                                            ))}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className={`rounded-2xl p-6 ${isDark ? 'bg-amber-900/20 border border-amber-500/40' : 'bg-amber-50 border border-amber-200'}`}>
                            <div className="flex items-start gap-3">
                                <Shield className={`w-6 h-6 mt-1 flex-shrink-0 ${isDark ? 'text-amber-400' : 'text-amber-600'}`} />
                                <div>
                                    <h4 className={`font-semibold mb-2 ${isDark ? 'text-amber-300' : 'text-amber-900'}`}>Legal Disclaimer</h4>
                                    <p className={`text-sm ${isDark ? 'text-amber-200' : 'text-amber-800'}`}>
                                        This tool provides general information only and is not legal advice. For specific business matters, please consult with a licensed attorney.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            {showAvatarCustomizer && (
                <AvatarCustomizerModal
                    ...
                    assistantName="Business Law Assistant"
                />
            )}
        </div>
    );
