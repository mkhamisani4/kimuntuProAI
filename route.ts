...
import { adminApp } from '@/packages/db/src/firebase/admin';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { jurisdiction, category, question, context, userId } = body;

    // Validate request
    if (!jurisdiction || !category || !question || !userId) {
      return NextResponse.json(
    ...
    const currentUsage = usageDoc.data()?.immigrationQueries || 0;
    const DAILY_LIMIT = 20; // Free tier limit

    if (currentUsage >= DAILY_LIMIT) {
      return NextResponse.json(
        { 
          error: `Daily limit of ${DAILY_LIMIT} immigration queries reached`,
          type: 'quota'
        },
        { status: 429 }
      );
    ...
    const knowledgeBase = jurisdiction === 'Both' 
      ...
      : IMMIGRATION_KNOWLEDGE_BASE[jurisdiction];

    const categoryInfo = knowledgeBase[category];

    if (!categoryInfo) {
      return NextResponse.json(
        { error: 'Category not found in knowledge base' },
        { status: 404 }
      );
    }
    ...
    const response = {
      ...
      meta: {
        model: 'gpt-4',
        tokensIn: estimateTokens(question + context),
        tokensOut: estimateTokens(answer),
        costCents: 2, // Simplified cost calculation
        latencyMs: 0, // Set by client
